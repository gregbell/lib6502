//! Comprehensive tests for the BCC (Branch if Carry Clear) instruction.
//!
//! Tests cover:
//! - Branch taken when carry flag is clear
//! - Branch not taken when carry flag is set
...
// ========== Basic BCC Operation Tests ==========

#[test]
fn test_bcc_branch_taken_forward() {
    let mut cpu = setup_cpu();

    // BCC +5 (0x90 0x05)
    // If carry clear, branch forward 5 bytes
...
fn test_bcc_branch_not_taken() {
    ...
    // Set carry flag (branch should NOT be taken)
    cpu.set_flag_c(true);
...
fn test_bcc_branch_backward() {
    ...
    // BCC -5 (0x90 0xFB) - 0xFB is -5 in signed 8-bit
    cpu.memory_mut().write(0x8000, 0x90);
...
#[test]
fn test_bcc_zero_offset() {
    let mut cpu = setup_cpu();

    // BCC +0 (0x90 0x00) - branch to next instruction
    cpu.memory_mut().write(0x8000, 0x90);
...
#[test]
fn test_bcc_page_cross_forward() {
...
#[test]
fn test_bcc_page_cross_backward() {
...
#[test]
fn test_bcc_no_page_cross_same_page() {
    let mut cpu = setup_cpu();

    // BCC +10 (0x90 0x0A) - stays within same page
    cpu.memory_mut().write(0x8000, 0x90);
...
#[test]
fn test_bcc_page_cross_boundary_exact() {
...
#[test]
fn test_bcc_preserves_all_flags() {
    let mut cpu = setup_cpu();
    ...
    // Set various flags (except carry which must be clear for branch)
    cpu.set_flag_c(false);
    cpu.set_flag_z(true);
    cpu.set_flag_i(true);
    cpu.set_flag_d(true);
...
#[test]
fn test_bcc_preserves_flags_branch_not_taken() {
    let mut cpu = setup_cpu();
    ...
    // Set flags with carry set (branch not taken)
    cpu.set_flag_c(true);
    cpu.set_flag_z(true);
    cpu.set_flag_n(true);
...
#[test]
fn test_bcc_max_forward_offset() {
    let mut cpu = setup_cpu();

    // BCC +127 (0x90 0x7F) - maximum positive offset
    cpu.memory_mut().write(0x8000, 0x90);
...
#[test]
fn test_bcc_max_backward_offset() {
    let mut cpu = setup_cpu();

    // BCC -128 (0x90 0x80) - maximum negative offset
    cpu.memory_mut().write(0x8000, 0x90);
...
#[test]
fn test_bcc_cycles_branch_not_taken() {
    let mut cpu = setup_cpu();

    cpu.memory_mut().write(0x8000, 0x90);
    cpu.memory_mut().write(0x8001, 0x10);

    cpu.set_flag_c(true); // Branch not taken
...
#[test]
fn test_bcc_cycles_branch_taken_no_page_cross() {
    let mut cpu = setup_cpu();

    cpu.memory_mut().write(0x8000, 0x90);
    cpu.memory_mut().write(0x8001, 0x10);

    cpu.set_flag_c(false); // Branch taken
...
#[test]
fn test_bcc_cycles_branch_taken_with_page_cross() {
    ...
    let mut cpu = CPU::new(memory);
    cpu.set_flag_c(false); // Branch taken with page cross
