//! Comprehensive tests for the BEQ (Branch if Equal) instruction.
//!
//! Tests cover:
//! - Branch taken when zero flag is set
//! - Branch not taken when zero flag is clear
...
fn test_beq_branch_not_taken() {
    ...
    // Clear zero flag (branch should NOT be taken)
...
fn test_beq_branch_backward() {
    ...
    // BEQ -5 (0xF0 0xFB) - 0xFB is -5 in signed 8-bit
    cpu.memory_mut().write(0x8000, 0xF0);
...
fn test_beq_zero_offset() {
    ...
    // BEQ +0 (0xF0 0x00) - branch to next instruction
    cpu.memory_mut().write(0x8000, 0xF0);
...
#[test]
fn test_beq_page_cross_forward() {
...
#[test]
fn test_beq_page_cross_backward() {
...
#[test]
fn test_beq_no_page_cross_same_page() {
    let mut cpu = setup_cpu();

    // BEQ +10 (0xF0 0x0A) - stays within same page
    cpu.memory_mut().write(0x8000, 0xF0);
...
#[test]
fn test_beq_page_cross_boundary_exact() {
...
#[test]
fn test_beq_preserves_all_flags() {
    let mut cpu = setup_cpu();
    ...
    // Set various flags (zero must be set for branch)
    cpu.set_flag_z(true);
    cpu.set_flag_c(true);
    cpu.set_flag_i(true);
    cpu.set_flag_d(true);
...
#[test]
fn test_beq_preserves_flags_branch_not_taken() {
    let mut cpu = setup_cpu();
    ...
    // Set flags with zero clear (branch not taken)
    cpu.set_flag_z(false);
    cpu.set_flag_c(true);
    cpu.set_flag_n(true);
...
#[test]
fn test_beq_max_forward_offset() {
    let mut cpu = setup_cpu();

    // BEQ +127 (0xF0 0x7F) - maximum positive offset
    cpu.memory_mut().write(0x8000, 0xF0);
...
#[test]
fn test_beq_max_backward_offset() {
    let mut cpu = setup_cpu();

    // BEQ -128 (0xF0 0x80) - maximum negative offset
    cpu.memory_mut().write(0x8000, 0xF0);
...
#[test]
fn test_beq_cycles_branch_not_taken() {
    let mut cpu = setup_cpu();

    cpu.memory_mut().write(0x8000, 0xF0);
    cpu.memory_mut().write(0x8001, 0x10);

    cpu.set_flag_z(false); // Branch not taken
...
#[test]
fn test_beq_cycles_branch_taken_no_page_cross() {
    let mut cpu = setup_cpu();

    cpu.memory_mut().write(0x8000, 0xF0);
    cpu.memory_mut().write(0x8001, 0x10);

    cpu.set_flag_z(true); // Branch taken
...
#[test]
fn test_beq_cycles_branch_taken_with_page_cross() {
    ...
    let mut cpu = CPU::new(memory);
    cpu.set_flag_z(true); // Branch taken with page cross
