//! Comprehensive tests for the TAY (Transfer Accumulator to Y) instruction.
//!
//! Tests cover:
//! - Basic TAY operation
//! - Flag updates (Z, N)
...
#[test]
fn test_tay_transfers_accumulator_to_y() {
    let mut cpu = setup_cpu();

    // TAY (0xA8)
    cpu.memory_mut().write(0x8000, 0xA8);
    cpu.set_a(0x55);
    cpu.set_y(0xFF); // Y has different value initially

    cpu.step().unwrap();

    assert_eq!(cpu.y(), 0x55); // Y now equals A
...
#[test]
fn test_tay_negative_flag_clear() {
    let mut cpu = setup_cpu();
...
#[test]
fn test_tay_negative_flag_with_0xff() {
    let mut cpu = setup_cpu();
...
#[test]
fn test_tay_preserves_accumulator() {
    let mut cpu = setup_cpu();
    ...
    assert_eq!(cpu.y(), 0x99); // Y should equal original A
}

#[test]
fn test_tay_preserves_x_register() {
    let mut cpu = setup_cpu();
...
#[test]
fn test_tay_preserves_carry_flag() {
    let mut cpu = setup_cpu();
...
#[test]
fn test_tay_preserves_overflow_flag() {
    let mut cpu = setup_cpu();
...
#[test]
fn test_tay_preserves_decimal_flag() {
    let mut cpu = setup_cpu();
...
#[test]
fn test_tay_preserves_interrupt_flag() {
    let mut cpu = setup_cpu();
...
#[test]
fn test_tay_sequence() {
    let mut cpu = setup_cpu();

    // Set up two TAY instructions
    cpu.memory_mut().write(0x8000, 0xA8);
    cpu.memory_mut().write(0x8001, 0xA8);
    ...
    // Change A and execute another TAY
    cpu.set_a(0x20);
    cpu.step().unwrap();
    assert_eq!(cpu.y(), 0x20); // Y updated with new A value
    assert_eq!(cpu.pc(), 0x8002);
}

#[test]
fn test_tay_overwrites_previous_y() {
    let mut cpu = setup_cpu();

    // TAY (0xA8)
    cpu.memory_mut().write(0x8000, 0xA8);
    cpu.set_a(0xAB);
    cpu.set_y(0xCD); // Y has a different value

    cpu.step().unwrap();

    assert_eq!(cpu.y(), 0xAB); // Y is overwritten with A
...
fn test_tay_cycle_count() {
    ...
    assert_eq!(cpu.cycles(), 2); // TAY takes 2 cycles
}

#[test]
fn test_tay_multiple_cycle_count() {
    let mut cpu = setup_cpu();

    // Set up 5 TAY instructions
    for i in 0..5 {
        cpu.memory_mut().write(0x8000 + i, 0xA8);
    }
    ...
    for i in 1..=5 {
        cpu.step().unwrap();
        assert_eq!(cpu.cycles(), (i * 2) as u64); // Each TAY takes 2 cycles
    }
}
...
#[test]
fn test_tay_program_counter_advance() {
    let mut cpu = setup_cpu();

    // TAY (0xA8) - single byte instruction
    cpu.memory_mut().write(0x8000, 0xA8);
    cpu.set_a(0x10);
...
#[test]
fn test_tay_clears_previous_z_flag() {
    let mut cpu = setup_cpu();
...
#[test]
fn test_tay_clears_previous_n_flag() {
    let mut cpu = setup_cpu();
