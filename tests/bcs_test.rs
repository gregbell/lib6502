//! Comprehensive tests for the BCS (Branch if Carry Set) instruction.
//!
//! Tests cover:
//! - Branch taken when carry flag is set
//! - Branch not taken when carry flag is clear
...
// ========== Basic BCS Operation Tests ==========

#[test]
fn test_bcs_branch_taken_forward() {
    let mut cpu = setup_cpu();

    // BCS +5 (0xB0 0x05)
    // If carry set, branch forward 5 bytes
...
fn test_bcs_branch_not_taken() {
    ...
    // Clear carry flag (branch should NOT be taken)
...
fn test_bcs_branch_backward() {
    ...
    // BCS -5 (0xB0 0xFB) - 0xFB is -5 in signed 8-bit
    cpu.memory_mut().write(0x8000, 0xB0);
...
#[test]
fn test_bcs_zero_offset() {
    let mut cpu = setup_cpu();

    // BCS +0 (0xB0 0x00) - branch to next instruction
    cpu.memory_mut().write(0x8000, 0xB0);
...
#[test]
fn test_bcs_page_cross_forward() {
...
#[test]
fn test_bcs_page_cross_backward() {
...
#[test]
fn test_bcs_no_page_cross_same_page() {
    let mut cpu = setup_cpu();

    // BCS +10 (0xB0 0x0A) - stays within same page
    cpu.memory_mut().write(0x8000, 0xB0);
...
#[test]
fn test_bcs_page_cross_boundary_exact() {
...
#[test]
fn test_bcs_preserves_all_flags() {
    let mut cpu = setup_cpu();
    ...
    // Set various flags (carry must be set for branch)
    cpu.set_flag_c(true);
    cpu.set_flag_z(true);
    cpu.set_flag_i(true);
    cpu.set_flag_d(true);
...
#[test]
fn test_bcs_preserves_flags_branch_not_taken() {
    let mut cpu = setup_cpu();
    ...
    // Set flags with carry clear (branch not taken)
    cpu.set_flag_c(false);
    cpu.set_flag_z(true);
    cpu.set_flag_n(true);
...
#[test]
fn test_bcs_max_forward_offset() {
    let mut cpu = setup_cpu();

    // BCS +127 (0xB0 0x7F) - maximum positive offset
    cpu.memory_mut().write(0x8000, 0xB0);
...
#[test]
fn test_bcs_max_backward_offset() {
    let mut cpu = setup_cpu();

    // BCS -128 (0xB0 0x80) - maximum negative offset
    cpu.memory_mut().write(0x8000, 0xB0);
...
#[test]
fn test_bcs_cycles_branch_not_taken() {
    let mut cpu = setup_cpu();

    cpu.memory_mut().write(0x8000, 0xB0);
    cpu.memory_mut().write(0x8001, 0x10);

    cpu.set_flag_c(false); // Branch not taken
...
#[test]
fn test_bcs_cycles_branch_taken_no_page_cross() {
    let mut cpu = setup_cpu();

    cpu.memory_mut().write(0x8000, 0xB0);
    cpu.memory_mut().write(0x8001, 0x10);

    cpu.set_flag_c(true); // Branch taken
...
#[test]
fn test_bcs_cycles_branch_taken_with_page_cross() {
    ...
    let mut cpu = CPU::new(memory);
    cpu.set_flag_c(true); // Branch taken with page cross
