//! Comprehensive tests for the BNE (Branch if Not Equal) instruction.
//!
//! Tests cover:
//! - Branch taken when zero flag is clear
//! - Branch not taken when zero flag is set
...
// ========== Basic BNE Operation Tests ==========

#[test]
fn test_bne_branch_taken_forward() {
    let mut cpu = setup_cpu();

    // BNE +5 (0xD0 0x05)
    // If zero flag clear, branch forward 5 bytes
    cpu.memory_mut().write(0x8000, 0xD0);
    cpu.memory_mut().write(0x8001, 0x05);

    // Ensure zero flag is clear
...
fn test_bne_branch_not_taken() {
    ...
    // Set zero flag (branch should NOT be taken)
    cpu.set_flag_z(true);
...
fn test_bne_branch_backward() {
    ...
    // BNE -5 (0xD0 0xFB) - 0xFB is -5 in signed 8-bit
    cpu.memory_mut().write(0x8000, 0xD0);
...
fn test_bne_zero_offset() {
    ...
    // BNE +0 (0xD0 0x00) - branch to next instruction
    cpu.memory_mut().write(0x8000, 0xD0);
...
#[test]
fn test_bne_page_cross_forward() {
...
#[test]
fn test_bne_page_cross_backward() {
...
#[test]
fn test_bne_no_page_cross_same_page() {
    let mut cpu = setup_cpu();

    // BNE +10 (0xD0 0x0A) - stays within same page
    cpu.memory_mut().write(0x8000, 0xD0);
...
#[test]
fn test_bne_page_cross_boundary_exact() {
...
#[test]
fn test_bne_preserves_all_flags() {
    let mut cpu = setup_cpu();
    ...
    // Set various flags (zero must be clear for branch)
    cpu.set_flag_z(false);
    cpu.set_flag_c(true);
    cpu.set_flag_i(true);
    cpu.set_flag_d(true);
...
#[test]
fn test_bne_preserves_flags_branch_not_taken() {
    let mut cpu = setup_cpu();
    ...
    // Set flags with zero set (branch not taken)
    cpu.set_flag_z(true);
    cpu.set_flag_c(true);
    cpu.set_flag_n(true);
...
#[test]
fn test_bne_max_forward_offset() {
    let mut cpu = setup_cpu();

    // BNE +127 (0xD0 0x7F) - maximum positive offset
    cpu.memory_mut().write(0x8000, 0xD0);
...
#[test]
fn test_bne_max_backward_offset() {
    let mut cpu = setup_cpu();

    // BNE -128 (0xD0 0x80) - maximum negative offset
    cpu.memory_mut().write(0x8000, 0xD0);
...
#[test]
fn test_bne_cycles_branch_not_taken() {
    let mut cpu = setup_cpu();

    cpu.memory_mut().write(0x8000, 0xD0);
    cpu.memory_mut().write(0x8001, 0x10);

    cpu.set_flag_z(true); // Branch not taken
...
#[test]
fn test_bne_cycles_branch_taken_no_page_cross() {
    let mut cpu = setup_cpu();

    cpu.memory_mut().write(0x8000, 0xD0);
    cpu.memory_mut().write(0x8001, 0x10);

    cpu.set_flag_z(false); // Branch taken
...
#[test]
fn test_bne_cycles_branch_taken_with_page_cross() {
    ...
    let mut cpu = CPU::new(memory);
    cpu.set_flag_z(false); // Branch taken with page cross
