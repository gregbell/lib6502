//! Comprehensive tests for the BPL (Branch if Positive) instruction.
//!
//! Tests cover:
//! - Branch taken when negative flag is clear
//! - Branch not taken when negative flag is set
...
// ========== Basic BPL Operation Tests ==========

#[test]
fn test_bpl_branch_taken_forward() {
    let mut cpu = setup_cpu();

    // BPL +5 (0x10 0x05)
    // If negative flag clear, branch forward 5 bytes
    cpu.memory_mut().write(0x8000, 0x10);
    cpu.memory_mut().write(0x8001, 0x05);

    // Ensure negative flag is clear
...
fn test_bpl_branch_not_taken() {
    ...
    // Set negative flag (branch should NOT be taken)
    cpu.set_flag_n(true);
...
fn test_bpl_branch_backward() {
    ...
    // BPL -5 (0x10 0xFB) - 0xFB is -5 in signed 8-bit
    cpu.memory_mut().write(0x8000, 0x10);
...
#[test]
fn test_bpl_zero_offset() {
    let mut cpu = setup_cpu();

    // BPL +0 (0x10 0x00) - branch to next instruction
    cpu.memory_mut().write(0x8000, 0x10);
...
#[test]
fn test_bpl_page_cross_forward() {
...
#[test]
fn test_bpl_page_cross_backward() {
...
#[test]
fn test_bpl_no_page_cross_same_page() {
    let mut cpu = setup_cpu();

    // BPL +10 (0x10 0x0A) - stays within same page
    cpu.memory_mut().write(0x8000, 0x10);
...
#[test]
fn test_bpl_page_cross_boundary_exact() {
...
#[test]
fn test_bpl_preserves_all_flags() {
    let mut cpu = setup_cpu();
    ...
    // Set various flags (negative must be clear for branch)
    cpu.set_flag_n(false);
    cpu.set_flag_z(true);
    cpu.set_flag_c(true);
    cpu.set_flag_i(true);
...
#[test]
fn test_bpl_preserves_flags_branch_not_taken() {
    let mut cpu = setup_cpu();
    ...
    // Set flags with negative set (branch not taken)
    cpu.set_flag_n(true);
    cpu.set_flag_z(true);
    cpu.set_flag_c(true);
...
#[test]
fn test_bpl_max_forward_offset() {
    let mut cpu = setup_cpu();

    // BPL +127 (0x10 0x7F) - maximum positive offset
    cpu.memory_mut().write(0x8000, 0x10);
...
#[test]
fn test_bpl_max_backward_offset() {
    let mut cpu = setup_cpu();

    // BPL -128 (0x10 0x80) - maximum negative offset
    cpu.memory_mut().write(0x8000, 0x10);
...
#[test]
fn test_bpl_cycles_branch_not_taken() {
    let mut cpu = setup_cpu();

    cpu.memory_mut().write(0x8000, 0x10);
    cpu.memory_mut().write(0x8001, 0x10);

    cpu.set_flag_n(true); // Branch not taken
...
#[test]
fn test_bpl_cycles_branch_taken_no_page_cross() {
    let mut cpu = setup_cpu();

    cpu.memory_mut().write(0x8000, 0x10);
    cpu.memory_mut().write(0x8001, 0x10);

    cpu.set_flag_n(false); // Branch taken
...
#[test]
fn test_bpl_cycles_branch_taken_with_page_cross() {
    ...
    let mut cpu = CPU::new(memory);
    cpu.set_flag_n(false); // Branch taken with page cross
