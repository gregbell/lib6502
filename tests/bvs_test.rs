//! Comprehensive tests for the BVS (Branch if Overflow Set) instruction.
//!
//! Tests cover:
//! - Branch taken when overflow flag is set
//! - Branch not taken when overflow flag is clear
...
// ========== Basic BVS Operation Tests ==========

#[test]
fn test_bvs_branch_taken_forward() {
    let mut cpu = setup_cpu();

    // BVS +5 (0x70 0x05)
    // If overflow flag set, branch forward 5 bytes
    cpu.memory_mut().write(0x8000, 0x70);
    cpu.memory_mut().write(0x8001, 0x05);

    // Ensure overflow flag is set
    cpu.set_flag_v(true);
...
fn test_bvs_branch_not_taken() {
    ...
    // Clear overflow flag (branch should NOT be taken)
...
fn test_bvs_branch_backward() {
    ...
    // BVS -5 (0x70 0xFB) - 0xFB is -5 in signed 8-bit
    cpu.memory_mut().write(0x8000, 0x70);
...
#[test]
fn test_bvs_zero_offset() {
    let mut cpu = setup_cpu();

    // BVS +0 (0x70 0x00) - branch to next instruction
    cpu.memory_mut().write(0x8000, 0x70);
...
#[test]
fn test_bvs_page_cross_forward() {
...
#[test]
fn test_bvs_page_cross_backward() {
...
#[test]
fn test_bvs_no_page_cross_same_page() {
    let mut cpu = setup_cpu();

    // BVS +10 (0x70 0x0A) - stays within same page
    cpu.memory_mut().write(0x8000, 0x70);
...
#[test]
fn test_bvs_page_cross_boundary_exact() {
...
#[test]
fn test_bvs_preserves_all_flags() {
    let mut cpu = setup_cpu();
    ...
    // Set various flags (overflow must be set for branch)
    cpu.set_flag_v(true);
    cpu.set_flag_c(true);
    cpu.set_flag_i(true);
    cpu.set_flag_d(true);
...
#[test]
fn test_bvs_preserves_flags_branch_not_taken() {
    let mut cpu = setup_cpu();
    ...
    // Set flags with overflow clear (branch not taken)
    cpu.set_flag_v(false);
    cpu.set_flag_c(true);
    cpu.set_flag_n(true);
...
#[test]
fn test_bvs_max_forward_offset() {
    let mut cpu = setup_cpu();

    // BVS +127 (0x70 0x7F) - maximum positive offset
    cpu.memory_mut().write(0x8000, 0x70);
...
#[test]
fn test_bvs_max_backward_offset() {
    let mut cpu = setup_cpu();

    // BVS -128 (0x70 0x80) - maximum negative offset
    cpu.memory_mut().write(0x8000, 0x70);
...
#[test]
fn test_bvs_cycles_branch_not_taken() {
    let mut cpu = setup_cpu();

    cpu.memory_mut().write(0x8000, 0x70);
    cpu.memory_mut().write(0x8001, 0x10);

    cpu.set_flag_v(false); // Branch not taken
...
#[test]
fn test_bvs_cycles_branch_taken_no_page_cross() {
    let mut cpu = setup_cpu();

    cpu.memory_mut().write(0x8000, 0x70);
    cpu.memory_mut().write(0x8001, 0x10);

    cpu.set_flag_v(true); // Branch taken
...
#[test]
fn test_bvs_cycles_branch_taken_with_page_cross() {
    ...
    let mut cpu = CPU::new(memory);
    cpu.set_flag_v(true); // Branch taken with page cross
