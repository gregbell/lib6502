//! Comprehensive tests for the BVC (Branch if Overflow Clear) instruction.
//!
//! Tests cover:
//! - Branch taken when overflow flag is clear
//! - Branch not taken when overflow flag is set
...
// ========== Basic BVC Operation Tests ==========

#[test]
fn test_bvc_branch_taken_forward() {
    let mut cpu = setup_cpu();

    // BVC +5 (0x50 0x05)
    // If overflow flag clear, branch forward 5 bytes
    cpu.memory_mut().write(0x8000, 0x50);
    cpu.memory_mut().write(0x8001, 0x05);

    // Ensure overflow flag is clear
...
fn test_bvc_branch_not_taken() {
    ...
    // Set overflow flag (branch should NOT be taken)
    cpu.set_flag_v(true);
...
fn test_bvc_branch_backward() {
    ...
    // BVC -5 (0x50 0xFB) - 0xFB is -5 in signed 8-bit
    cpu.memory_mut().write(0x8000, 0x50);
...
#[test]
fn test_bvc_zero_offset() {
    let mut cpu = setup_cpu();

    // BVC +0 (0x50 0x00) - branch to next instruction
    cpu.memory_mut().write(0x8000, 0x50);
...
#[test]
fn test_bvc_page_cross_forward() {
...
#[test]
fn test_bvc_page_cross_backward() {
...
#[test]
fn test_bvc_no_page_cross_same_page() {
    let mut cpu = setup_cpu();

    // BVC +10 (0x50 0x0A) - stays within same page
    cpu.memory_mut().write(0x8000, 0x50);
...
#[test]
fn test_bvc_page_cross_boundary_exact() {
...
#[test]
fn test_bvc_preserves_all_flags() {
    let mut cpu = setup_cpu();
    ...
    // Set various flags (overflow must be clear for branch)
    cpu.set_flag_v(false);
    cpu.set_flag_c(true);
    cpu.set_flag_i(true);
    cpu.set_flag_d(true);
...
#[test]
fn test_bvc_preserves_flags_branch_not_taken() {
    let mut cpu = setup_cpu();
    ...
    // Set flags with overflow set (branch not taken)
    cpu.set_flag_v(true);
    cpu.set_flag_c(true);
    cpu.set_flag_n(true);
...
#[test]
fn test_bvc_max_forward_offset() {
    let mut cpu = setup_cpu();

    // BVC +127 (0x50 0x7F) - maximum positive offset
    cpu.memory_mut().write(0x8000, 0x50);
...
#[test]
fn test_bvc_max_backward_offset() {
    let mut cpu = setup_cpu();

    // BVC -128 (0x50 0x80) - maximum negative offset
    cpu.memory_mut().write(0x8000, 0x50);
...
#[test]
fn test_bvc_cycles_branch_not_taken() {
    let mut cpu = setup_cpu();

    cpu.memory_mut().write(0x8000, 0x50);
    cpu.memory_mut().write(0x8001, 0x10);

    cpu.set_flag_v(true); // Branch not taken
...
#[test]
fn test_bvc_cycles_branch_taken_no_page_cross() {
    let mut cpu = setup_cpu();

    cpu.memory_mut().write(0x8000, 0x50);
    cpu.memory_mut().write(0x8001, 0x10);

    cpu.set_flag_v(false); // Branch taken
...
#[test]
fn test_bvc_cycles_branch_taken_with_page_cross() {
    ...
    let mut cpu = CPU::new(memory);
    cpu.set_flag_v(false); // Branch taken with page cross
