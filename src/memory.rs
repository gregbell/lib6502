...
pub trait MemoryBus {
    ...
    /// Checks if the NMI (Non-Maskable Interrupt) line is active.
    ///
    /// This method returns `true` if any memory-mapped device has a pending
    /// NMI request. Unlike IRQ, NMI cannot be disabled by the I flag and is
    /// edge-triggered (responds to falling edge transitions).
    ///
    /// # Hardware Semantics
    ///
    /// The NMI line on the 6502 is **edge-triggered** (not level-sensitive like IRQ):
    ///
    /// - **Edge-triggered**: NMI responds to HIGH→LOW transitions
    /// - **Cannot be disabled**: The I flag has no effect on NMI
    /// - **Higher priority**: NMI is serviced before IRQ if both pending
    /// - **Vector at $FFFA**: Uses different interrupt vector than IRQ ($FFFE)
    ///
    /// # C64 Usage
    ///
    /// On the Commodore 64, NMI is generated by:
    /// - CIA2 interrupts (directly wired to NMI line)
    /// - RESTORE key press (directly pulls NMI line low)
    ///
    /// # Default Implementation
    ///
    /// Returns `false` (no NMI) for simple memory implementations
    /// that don't support NMI-generating devices.
    ///
    /// # Returns
    ///
    /// - `true` if NMI line is currently active (low)
    /// - `false` if NMI line is inactive (high)
    ///
    /// # Examples
    ///
    /// ```
    ...
    ///
    /// let mem = FlatMemory::new();
    /// // FlatMemory has no NMI-capable devices
    /// assert_eq!(mem.nmi_active(), false);
    /// ```
    fn nmi_active(&self) -> bool {
        false // Default: no NMI
    }
}
