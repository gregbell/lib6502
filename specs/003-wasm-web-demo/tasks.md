...
- [X] T023 Create manual test HTML: Verify WASM API from browser console (create emulator, load program, step, read registers)
...
- [X] T024 [US6] Configure GitHub Pages in repository settings: Enable Pages, set source to main branch /demo directory
- [X] T025 [US6] Update .github/workflows/deploy-demo.yml: Add WASM build step and Pages deployment step
- [X] T026 [US6] Create demo/index.html skeleton: Basic structure with title, empty panels for editor and CPU state
- [X] T027 [US6] Add WASM module loading to demo/index.html: Import init and Emulator6502 from lib6502_wasm.js
- [X] T028 [US6] Implement basic WASM initialization in demo/app.js: await init(), create emulator instance
- [X] T029 [US6] Test local deployment: Run `python3 -m http.server 8000 -d demo/` and verify page loads
- [X] T030 [US6] Push to GitHub and verify Pages deployment: Check workflow runs successfully
...
- [X] T031 [P] [US3] Implement RegisterDisplay component in demo/components/registers.js: Display A, X, Y, PC, SP with 2/4-digit hex formatting
- [X] T032 [P] [US3] Implement FlagsDisplay component in demo/components/flags.js: Display N, V, D, I, Z, C as boolean indicators
- [X] T033 [US3] Add cycle counter display to RegisterDisplay: Show cycles with thousands separators
- [X] T034 [US3] Create CSS styling for registers and flags in demo/styles.css: Monospace font, clear labels, visual indicators for flags

### Integration

- [X] T035 [US3] Add register/flag containers to demo/index.html: Right panel with divs for registers and flags
- [X] T036 [US3] Implement updateCPUDisplay() in demo/app.js: Fetch all registers/flags from WASM, call component update methods
- [X] T037 [US3] Add requestAnimationFrame update loop in demo/app.js: Call updateCPUDisplay() at 60fps
- [X] T038 [US3] Test CPU state display: Load simple program, step through, verify registers update correctly
...
- [X] T039 [P] [US1] Implement CodeEditor component in demo/components/editor.js: Textarea with getValue() and setValue() methods
- [X] T040 [P] [US1] Implement Asm6502Highlighter in demo/components/editor.js: Regex-based syntax highlighting for mnemonics, hex, comments, labels
- [X] T041 [US1] Add syntax highlighting CSS to demo/styles.css: Colors for instructions, operands, comments, labels
- [X] T042 [US1] Integrate editor into demo/index.html: Left panel with textarea and highlighted pre element

### Control Panel Component

- [X] T043 [P] [US1] Implement ControlPanel component in demo/components/controls.js: Assemble, Run, Step, Stop, Reset buttons
- [X] T044 [P] [US1] Implement button state management in ControlPanel: setMode() to enable/disable based on execution state
- [X] T045 [US1] Add control panel to demo/index.html: Below editor with button group
- [X] T046 [US1] Style control panel in demo/styles.css: Clear buttons with hover states

### Assembly Workflow

- [X] T047 [US1] Implement handleAssemble() in demo/app.js: Get code from editor, call emu.assemble_and_load(), handle errors
- [X] T048 [P] [US1] Implement ErrorDisplay component in demo/components/error.js: Show assembly errors with line numbers
- [X] T049 [US1] Implement assembled program persistence: Store assembled state, prevent re-assembly until code changes or reset
- [X] T050 [US1] Wire Assemble button event: Listen for 'assemble-clicked', call handleAssemble()

### Execution Workflow

- [X] T051 [US1] Implement handleRun() in demo/app.js: Set mode to 'running', call emu.run_for_cycles() in animation loop
- [X] T052 [US1] Implement program completion detection: Check if PC reached end of code range or BRK instruction
- [X] T053 [US1] Wire Run button event: Listen for 'run-clicked', call handleRun()
- [X] T054 [US1] Update UI during execution: Call updateCPUDisplay() every frame while running

### Validation

- [X] T055 [US1] Test write-assemble-run workflow: Type `LDA #$42`, click Assemble, click Run, verify A=0x42
- [X] T056 [US1] Test assembly error handling: Type invalid code, click Assemble, verify error displays with line number
- [X] T057 [US1] Test program persistence: Assemble once, click Run multiple times, verify no re-assembly
...
- [X] T058 [US2] Implement handleStep() in demo/app.js: Call emu.step() once, update display, handle errors
- [X] T059 [US2] Wire Step button event: Listen for 'step-clicked', call handleStep()
- [X] T060 [US2] Implement step debouncing in ControlPanel: Prevent rapid clicks during step execution (<100ms)
- [X] T061 [US2] Update button states during step: Disable Step briefly during execution, re-enable when complete

### Reset Functionality

- [X] T062 [US2] Implement handleReset() in demo/app.js: Call emu.reset(), set PC to program start, update display
- [X] T063 [US2] Wire Reset button event: Listen for 'reset-clicked', call handleReset()
- [X] T064 [US2] Test reset behavior: Step through program, click Reset, verify all registers return to initial state and PC returns to start

### Validation

- [X] T065 [US2] Test step-through execution: Load multi-instruction program (5+ instructions), step through completely, verify PC increments correctly
- [X] T066 [US2] Test per-instruction register changes: Use program that modifies different registers, step and observe only affected register changes
...
- [X] T067 [P] [US5] Implement MemoryViewer component in demo/components/memory.js: Virtual scrolling with 16 bytes/row, 4096 rows total
- [X] T068 [P] [US5] Implement virtual scroll rendering in MemoryViewer: Render only visible rows (~25), update on scroll
- [X] T069 [P] [US5] Implement memory row rendering: Address (4-digit hex) + 16 hex bytes + ASCII column
- [X] T070 [P] [US5] Add memory viewer container to demo/index.html: Right panel, stacked below registers/flags or side-by-side
- [X] T071 [P] [US5] Style memory viewer in demo/styles.css: Monospace, clear columns, scrollable with fixed height

### Memory Navigation

- [X] T072 [US5] Implement jumpToAddress(addr) in MemoryViewer: Scroll to row containing address, highlight briefly
- [X] T073 [US5] Add address jump input to memory viewer UI: Text input for hex address, button to jump
- [X] T074 [US5] Wire address jump: Listen for input, parse hex, call jumpToAddress()

### Real-Time Updates

- [X] T075 [US5] Implement dirty byte tracking in MemoryViewer: Compare new memory with cached, mark changed bytes
- [X] T076 [US5] Implement updateMemory() in demo/app.js: Fetch visible memory pages from WASM, pass to MemoryViewer
- [X] T077 [US5] Add memory viewer to update loop: Call updateMemory() every frame (60fps), only fetch visible pages
- [X] T078 [US5] Implement changed byte highlighting: CSS flash animation on dirty bytes, 1s fade-out
- [X] T079 [US5] Style dirty byte highlighting in demo/styles.css: Red/orange background flash

### Validation

- [X] T080 [US5] Test memory display: Load program at $0600, verify hex dump shows program bytes
- [X] T081 [US5] Test memory updates: Run `STA $1000` instruction, verify $1000 updates and highlights
- [X] T082 [US5] Test address navigation: Jump to $0200, $FFFF, verify scroll and highlight work
- [X] T083 [US5] Test virtual scrolling performance: Scroll through entire 64KB range, verify 60fps maintained
...
- [X] T084 [P] [US4] Create counter example: `demo/examples/counter.asm` - Simple counter from 0 to 255 in accumulator
- [X] T085 [P] [US4] Create Fibonacci example: `demo/examples/fibonacci.asm` - Calculate Fibonacci sequence
- [X] T086 [P] [US4] Create stack demo example: `demo/examples/stack-demo.asm` - Demonstrate PHA/PLA stack operations
- [X] T087 [P] [US4] Add example program metadata: Define EXAMPLES array in demo/app.js with id, name, description, source

### Example Selector Component

- [X] T088 [US4] Implement ExampleSelector component in demo/components/examples.js: Dropdown with example programs
- [X] T089 [US4] Add example selector to demo/index.html: Above or within editor panel
- [X] T090 [US4] Wire example loading: Listen for 'example-loaded' event, call editor.setValue(), reset emulator
- [X] T091 [US4] Test example loading: Click each example, verify code loads into editor

### Validation

- [X] T092 [US4] Test counter example execution: Load counter, run for 1000 cycles, verify A increments
- [X] T093 [US4] Test Fibonacci example execution: Load Fibonacci, step through, verify correct sequence in registers
- [X] T094 [US4] Test stack demo execution: Load stack demo, step through, observe SP changes and memory in stack page
...
- [X] T095 Add speed selector dropdown to ControlPanel: Options for 0.5, 1.0, 1.79, 2.0 MHz, unlimited
- [X] T096 Implement getSpeed() and setSpeed() in ControlPanel: Return current speed, update dropdown
- [X] T097 Wire speed change event: Listen for 'speed-changed', calculate cycles per frame
- [X] T098 Implement cycle-accurate timing in handleRun(): Calculate target cycles based on speed and 60fps, call run_for_cycles()

### Timing Implementation

- [X] T099 Add calculateCyclesPerFrame() utility: Convert MHz to cycles per frame at 60fps
- [X] T100 Default to 1 MHz on page load: Set initial speed selector to 1 MHz
- [X] T101 Allow speed changes during execution: Update cycles per frame without disrupting program state
- [X] T102 Test timing accuracy: Run at 1 MHz for 1 second, verify ~1,000,000 cycles executed (±5%)

### Validation

- [X] T103 Test 0.5 MHz (slow): Verify execution is visibly slower than 1 MHz
- [X] T104 Test 1 MHz (default): Verify authentic timing matches spec (16,667 cycles/frame at 60fps)
- [X] T105 Test unlimited speed: Verify program executes as fast as possible (100,000+ cycles/frame)
- [X] T106 Test speed switching during execution: Run program, change speed mid-execution, verify no crash
...
- [X] T107 Implement runtime error display: Catch execution errors, show error modal with PC and opcode
- [X] T108 Handle empty program execution: Disable Run/Step if no program assembled, show message
- [X] T109 Handle browser compatibility: Display clear message if WebAssembly not supported
- [X] T110 Test unimplemented opcode error: Execute unimplemented instruction, verify error displays

### Performance Optimization

- [X] T111 Optimize memory viewer updates: Only re-render changed bytes, not entire visible window
- [X] T112 Implement Stop button functionality: Halt run loop, return to idle mode
- [X] T113 Add FPS counter (debug mode): Display actual frame rate for performance monitoring
- [X] T114 Profile performance: Use browser DevTools, ensure 60fps maintained during Run mode

### UI Polish

- [X] T115 Add keyboard shortcuts: Space=Step, R=Run, S=Stop, Esc=Reset
- [X] T116 Improve visual feedback: Button press animations, loading states
- [X] T117 Add help/documentation panel: Brief instructions for using the demo
- [X] T118 Responsive layout adjustments: Ensure panels stack appropriately on smaller screens (optional mobile support)

### Documentation

- [X] T119 Add code comments: Document all components, explain non-obvious logic
- [X] T120 Create README for demo/: Explain how to build and deploy
- [X] T121 Update main project README: Add link to live demo, screenshot

### Final Validation

- [X] T122 Cross-browser testing: Verify demo works in Chrome, Firefox, Safari
- [X] T123 Load performance test: Verify demo loads in <3s on broadband
- [X] T124 Stress test: Run infinite loop program, verify Stop button halts within 1s
- [X] T125 Full end-to-end test: Complete all user stories' acceptance scenarios
