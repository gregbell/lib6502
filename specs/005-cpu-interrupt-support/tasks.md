...
- [ ] T001 Verify existing project structure matches plan.md (src/, tests/, examples/)
- [ ] T002 Create new module file src/devices/interrupts.rs for InterruptDevice trait
...
- [ ] T003 [P] Add InterruptDevice trait to src/devices/interrupts.rs with has_interrupt() method
- [ ] T004 [P] Add irq_pending field (bool) to CPU struct in src/cpu.rs
- [ ] T005 Add irq_active() method to MemoryBus trait in src/memory.rs that returns bool
- [ ] T006 Implement irq_active() in MappedMemory (src/devices/mod.rs) to check all devices' has_interrupt()
- [ ] T007 Export InterruptDevice trait from src/lib.rs public API
- [ ] T008 Add documentation comments to InterruptDevice trait explaining hardware-accurate behavior
...
- [ ] T009 [US1] Add interrupt checking to CPU::step() in src/cpu.rs after instruction execution
- [ ] T010 [US1] Implement check_irq_line() helper in src/cpu.rs that calls memory.irq_active() and updates irq_pending
- [ ] T011 [US1] Implement should_service_interrupt() helper in src/cpu.rs (returns irq_pending && !flag_i)
- [ ] T012 [US1] Implement service_interrupt() method in src/cpu.rs with 7-cycle IRQ sequence
- [ ] T013 [US1] In service_interrupt(): Push PC high byte to stack (1 cycle) in src/cpu.rs
- [ ] T014 [US1] In service_interrupt(): Push PC low byte to stack (1 cycle) in src/cpu.rs
- [ ] T015 [US1] In service_interrupt(): Push status register to stack (1 cycle) in src/cpu.rs
- [ ] T016 [US1] In service_interrupt(): Set I flag to prevent nested interrupts in src/cpu.rs
- [ ] T017 [US1] In service_interrupt(): Read IRQ vector low byte from 0xFFFE (1 cycle) in src/cpu.rs
- [ ] T018 [US1] In service_interrupt(): Read IRQ vector high byte from 0xFFFF (1 cycle) in src/cpu.rs
- [ ] T019 [US1] In service_interrupt(): Set PC to vector address (2 cycles) in src/cpu.rs
- [ ] T020 [US1] In service_interrupt(): Add exactly 7 to cycle counter in src/cpu.rs
- [ ] T021 [US1] Add documentation comments to service_interrupt() explaining cycle breakdown

#### Example Timer Device

- [ ] T022 [P] [US1] Create TimerDevice struct in examples/interrupt_device.rs with interrupt_pending field
- [ ] T023 [P] [US1] Implement Device trait for TimerDevice in examples/interrupt_device.rs (size() returns 4)
- [ ] T024 [P] [US1] Implement InterruptDevice trait for TimerDevice in examples/interrupt_device.rs
- [ ] T025 [P] [US1] Implement MemoryBus trait for TimerDevice with 4 memory-mapped registers in examples/interrupt_device.rs
- [ ] T026 [US1] Add tick() method to TimerDevice that sets interrupt_pending when counter expires in examples/interrupt_device.rs
- [ ] T027 [US1] Implement STATUS register (offset 0) in TimerDevice showing interrupt_pending in bit 7 in examples/interrupt_device.rs
- [ ] T028 [US1] Implement CONTROL register (offset 1) in TimerDevice that clears interrupt on write in examples/interrupt_device.rs
- [ ] T029 [US1] Implement COUNTER_LO register (offset 2) in TimerDevice for read-only counter value in examples/interrupt_device.rs
- [ ] T030 [US1] Implement COUNTER_HI register (offset 3) in TimerDevice for read-only counter value in examples/interrupt_device.rs

#### Integration & Validation

- [ ] T031 [US1] Create integration test in tests/integration/test_interrupts.rs verifying single device interrupt
- [ ] T032 [US1] Add test case: device asserts interrupt, CPU services it at next instruction boundary in tests/integration/test_interrupts.rs
- [ ] T033 [US1] Add test case: I flag set, interrupt not serviced until I flag cleared in tests/integration/test_interrupts.rs
- [ ] T034 [US1] Add test case: ISR reads device status register, device clears interrupt in tests/integration/test_interrupts.rs
- [ ] T035 [US1] Add test case: verify exactly 7 cycles consumed by interrupt sequence in tests/integration/test_interrupts.rs
- [ ] T036 [US1] Create example program in examples/interrupt_device.rs demonstrating timer interrupt workflow
- [ ] T037 [US1] Add ISR example code (6502 assembly comments) showing how to poll and acknowledge device in examples/interrupt_device.rs
...
- [ ] T054 [P] Update CLAUDE.md with interrupt support implementation notes and example usage
- [ ] T055 [P] Add module-level documentation to src/devices/interrupts.rs explaining interrupt model
- [ ] T056 [P] Add README or comments to examples/interrupt_device.rs explaining example structure
- [ ] T057 Run cargo clippy and fix any warnings in interrupt-related code
- [ ] T058 Run cargo fmt on all modified files
